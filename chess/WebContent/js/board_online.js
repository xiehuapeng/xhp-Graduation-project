...
function Board(container, images) {
  ...
  for (var sq = 0; sq < 256; sq ++) {
	...
	// 每个棋盘区域的点击事件，sq_为具体点击的区域。
	    img.onmousedown = function clickimg(sq_) {
	        return function() {
	          this_.clickSquare(sq_);
	          if(this_.busy!=true){
	          	
	          	socket.emit('move', 254-sq_);//点击对应区域时将对应数值发送给服务器
	          	
	          	console.log(sq_);
	          	
	          	console.log(FILE_X(sq_));
	          	console.log(RANK_Y(sq_));
	    ...
	    //this.imgSquares[164].onmousedown(164);
...
Board.prototype.postMate = function(computerMove) {
  alertDelay(computerMove ? "请再接再厉！" : "游戏结束！");
  this.busy = false;
}
